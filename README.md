...
- **Note:**
  - If you are using styled-components or normal HTML elements in your components then you should use the test id attribute as `data-testid` for all the elements wherever needed.
    - Example :- `<RestaurantItemCard data-testid="restaurant-item" />` or `<div data-testid="restaurant-item" className="restaurant-item"/>`
- Routes:
...
- Home:
  ...
  - The Restaurant List Page number should contain the test id value as `active-page-number`
- Footer:
  ...
  - The Pintrest social icon should contain the test id value as `pintrest-social-icon`.
  - The Instagram social icon should contain the test id value as `Instagram-social-icon`.
  - The Twitter social icon should contain the test id value as `twitter-social-icon`.
  - The Facebook social icon should contain the test id value as `facebook-social-icon`.
- Restaurant Details:
...
- For Mini Projects, you can publish/deploy the code using `ccbp publish RJSCPP63AV domain_name.ccbp.tech`. So that our team will be able to see that code and deployed URL( Ex: something.ccbp.tech) and will share feedback to you if required.
